% Sequential AtD variant 
function [ negLLAtD, AtDParam ] = LLAtDSeq_N_NoA(PtsErr, AvgSeq, PtsErrEarly, PtsErrLate, N  )
% This AtD variant does not assume a set relationship between set size and
...
[AtDParam, negLLAtD]= fminsearch(@(B) (MLEAtD(PtsErr{1,1}(:,end)',0,B(1),B(5)) + ...
                     ...
                     -sum(log((normpdf(AvgSeq{1,1}(:,end)',mean(AvgSeq{1,1}(:,end)'),sqrt( B(2)  +  ((N-1)/N)^2*(.5*B(1))   +.5*B(1) )))))+...             
                     -sum(log((normpdf(AvgSeq{2,1}(:,end)',mean(AvgSeq{2,1}(:,end)'),sqrt( B(2)   +  ((N-1)/N)^2*(3*B(1))    + 3*B(1) )))))+...             
