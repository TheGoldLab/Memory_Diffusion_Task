function [ PtsErrSIM, AvgSIM ] = GenIndAvgDtA(AtDParam,N, numSamples , PtsErr, AvgErr)
% This function takes AtD parameters, a set size (N), and a number of
% trials (numSamples) and siumulates data from the DtA model.   It also
...
for delay=1:3
   ...
   AvgSIM{delay,2,2}=AvgMn+(AtDParam(4)+delays(delay)*AtDParam(1)*N^AtDParam(5)/N)^.5*randn(numSamples,1);
