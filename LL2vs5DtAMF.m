%%DtA true version
function [ negLLAtD, AtDParam,SEMS_ ] = LL2vs5DtAMF(PtsErr, Block3, N  )
...
[AtDParam, negLLAtD,~,~,~,~,HESSIAN]= fmincon(@(B) (MLEAtDFreeMean(PtsErr{1,1}(1,:),0,B(2),B(1)) + ...
                     ...
                     MLEAtDFreeMean(Block3{1,1}(:,end)',0,B(4),B(1)*N^B(5)/N) + ...
                     MLEAtDFreeMean(Block3{2,1}(:,end)',1,B(4),B(1)*N^B(5)/N) + ...
                     MLEAtDFreeMean(Block3{3,1}(:,end)',6,B(4),B(1)*N^B(5)/N)), [3,10,10,10,.25],[],[],[],[],[],[],[], options);
