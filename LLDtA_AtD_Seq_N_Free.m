...
function [ negLLAtD, AtDParam, SEMS_ ] = LLDtA_AtD_Seq_N_Free(PtsErr, AvgSeq, PtsErrEarly, PtsErrLate, N  )
...
[AtDParam, negLLAtD,~,~,~,~,HESSIAN]= fmincon(@(B) (MLEAtD(PtsErr{1,1}(:,end)',0,B(2),B(1)) + ...
                     ...
                     -sum(log((normpdf(AvgSeq{1,1}(:,end)',mean(AvgSeq{1,1}(:,end)'),sqrt( B(3)+ ( (N-1)*(.5*B(1)*(N-1)^B(5)))/N^2 + .5*B(1))))))+...             
                     -sum(log((normpdf(AvgSeq{2,1}(:,end)',mean(AvgSeq{2,1}(:,end)'),sqrt( B(3)+ ( (N-1)* (3*B(1)*(N-1)^B(5)))/N^2 + 3*B(1))))))+...             
