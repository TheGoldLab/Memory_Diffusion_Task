...
let expName = 'MGSblock5-final';  // from the Builder filename that created this script
...
flowScheduler.add(Inst3_5RoutineBegin());
flowScheduler.add(Inst3_5RoutineEachFrame());
flowScheduler.add(Inst3_5RoutineEnd());
...
function experimentInit() {
  ...
  DemocloseOthers = new visual.TextStim({
    win: psychoJS.window,
    name: 'DemocloseOthers',
  ...
  fake2s = new visual.Polygon ({
    win: psychoJS.window, name: 'fake2s', 
    edges: 100, size:[0.015, 0.015],
    ori: 0, pos: [0, 0],
    lineWidth: 1, lineColor: new util.Color(1.0),
    fillColor: new util.Color(1.0),
  ...
  fake3s = new visual.Polygon ({
    win: psychoJS.window, name: 'fake3s', 
    edges: 100, size:[0.015, 0.015],
    ori: 0, pos: [0, 0],
    lineWidth: 1, lineColor: new util.Color(democols[2]),
    fillColor: new util.Color(democols[2]),
    opacity: 1, depth: -6, interpolate: true,
  });
  
  fake4s = new visual.Polygon ({
    win: psychoJS.window, name: 'fake4s', 
    edges: 100, size:[0.015, 0.015],
    ori: 0, pos: [0, 0],
    lineWidth: 1, lineColor: new util.Color(1.0),
    fillColor: new util.Color(1.0),
  ...
  fake5s = new visual.Polygon ({
    win: psychoJS.window, name: 'fake5s', 
    edges: 100, size:[0.015, 0.015],
    ori: 0, pos: [0, 0],
    lineWidth: 1, lineColor: new util.Color(democols[4]),
    fillColor: new util.Color(democols[4]),
  ...
  Inst3_5Clock = new util.Clock();
  key_resp = new core.Keyboard({psychoJS: psychoJS, clock: new util.Clock(), waitForStart: true});
  
  demolasttarg = new visual.TextStim({
    win: psychoJS.window,
    name: 'demolasttarg',
  ...
  demohowmove = new visual.TextStim({
    win: psychoJS.window,
    name: 'demohowmove',
    text: 'Please use the mouse\nto move the circle to the location on the ring\nwhere you remember the matching color target',
    font: 'Arial',
    units: undefined, 
    pos: [0, 0], height: 0.03,  wrapWidth: undefined, ori: 0,
    color: new util.Color('white'),  opacity: 1,
...
function InstCloseRoutineBegin(trials) {
  return function () {
    ...
    // keep track of which components have finished
    InstCloseComponents = [];
    InstCloseComponents.push(key_resp_10);
    InstCloseComponents.push(DemocloseOthers);
    
    for (const thisComponent of InstCloseComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
...
function InstCloseRoutineEachFrame(trials) {
  return function () {
    ...
    if (t >= 0.0 && DemocloseOthers.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      DemocloseOthers.tStart = t;  // (not accounting for frame time here)
...
function Inst2RoutineBegin(trials) {
  return function () {
    ...
    fake2s.setFillColor(new util.Color(democols[1]));
    fake2s.setLineColor(new util.Color(democols[1]));
    fake3s.setPos([pos3x, pos3y]);
    fake4s.setPos([pos4x, pos4y]);
    fake4s.setFillColor(new util.Color(democols[3]));
    fake4s.setLineColor(new util.Color(democols[3]));
    ...
    Inst2Components.push(fake2s);
    Inst2Components.push(fake3s);
    Inst2Components.push(fake4s);
    Inst2Components.push(fake5s);
...
function Inst2RoutineEachFrame(trials) {
  return function () {
    ...
    // *fake2s* updates
    if (t >= 0.0 && fake2s.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      fake2s.tStart = t;  // (not accounting for frame time here)
    ...
    // *fake3s* updates
    if (t >= 0.0 && fake3s.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      fake3s.tStart = t;  // (not accounting for frame time here)
    ...
    // *fake4s* updates
    if (t >= 0.0 && fake4s.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      fake4s.tStart = t;  // (not accounting for frame time here)
    ...
    // *fake5s* updates
    if (t >= 0.0 && fake5s.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      fake5s.tStart = t;  // (not accounting for frame time here)
...
var Inst3_5Components;
function Inst3_5RoutineBegin(trials) {
  return function () {
    //------Prepare to start Routine 'Inst3_5'-------
    t = 0;
    Inst3_5Clock.reset(); // clock
    frameN = -1;
    // update component parameters for each repeat
    ...
    // keep track of which components have finished
    Inst3_5Components = [];
    Inst3_5Components.push(key_resp);
    Inst3_5Components.push(demolasttarg);
    
    for (const thisComponent of Inst3_5Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
...
function Inst3_5RoutineEachFrame(trials) {
  return function () {
    //------Loop for each frame of Routine 'Inst3_5'-------
    let continueRoutine = true; // until we're told otherwise
    // get current time
    ...
    if (t >= 0.0 && demolasttarg.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      demolasttarg.tStart = t;  // (not accounting for frame time here)
...
function Inst3_5RoutineEnd(trials) {
  return function () {
...
function Inst5RoutineBegin(trials) {
  return function () {
    ...
    for (const thisComponent of Inst5Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
...
function Inst6RoutineBegin(trials) {
  return function () {
    ...
    for (const thisComponent of Inst6Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
...
function Inst7RoutineBegin(trials) {
  return function () {
    ...
    for (const thisComponent of Inst7Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
...
function trialRoutineEachFrame(trials) {
  return function () {
    ...
    // *Targ2* updates
    if (t >= (time2 + 0.25) && Targ2.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    frameRemains = (time2 + 0.25) + 0.5 - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (Targ2.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      Targ2.setAutoDraw(false);
    }
    
    // *Targ3* updates
    if (t >= (time3 + 0.25) && Targ3.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    frameRemains = (time3 + 0.25) + 0.5 - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (Targ3.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      Targ3.setAutoDraw(false);
    }
    
    // *Targ4* updates
    if (t >= (time4 + 0.25) && Targ4.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    frameRemains = (time4 + 0.25) + 0.5 - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (Targ4.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      Targ4.setAutoDraw(false);
    }
    
    // *Targ5* updates
    if (t >= (time5 + 0.25) && Targ5.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    frameRemains = (time5 + 0.25) + 0.5 - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
...
function trialRoutineEnd(trials) {
  return function () {
    ...
    psychoJS.experiment.addData("exptType", 1);
